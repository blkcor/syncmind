...
- [ ] 实现配置加载：从 `<config-dir>/syncmind/config.toml` 读取用户配置。
...
- [ ] **ONNX Embedder**: 使用 `ort` crate 加载内置（或首次下载缓存到 `<data-dir>/syncmind/models/`）的 `bge-small-en-v1.5` ONNX 模型。
...
**Acceptance Criteria:**
- [ ] 使用 `rusqlite` 连接本地数据库 `<data-dir>/syncmind/syncmind.db`。
...
- **FR-6:** 系统必须使用 SQLite + `sqlite-vec` 作为本地向量与元数据存储引擎，数据文件存储在 `<data-dir>/syncmind/`。
...
- **日志:** 使用 `tracing` 结构化日志，日志文件可轮转写入 `<data-dir>/syncmind/logs/`。
