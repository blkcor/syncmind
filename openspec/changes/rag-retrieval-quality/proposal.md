...
- `score-threshold`: a default relevance threshold of 0.4 filters out low-quality matches; hybrid search applies threshold on fused scores.

### Modified Capabilities
- `code-chunker`: chunk content no longer contains hard-concatenated signatures; context is injected through metadata for embedding only.
...
- **New dependencies (Rust, `core/rag-engine/Cargo.toml`)**: `tree-sitter-css` (MIT). License audit: MIT.
- **Build & test gates**: `cd core && cargo check && cargo clippy --all-targets -- -D warnings && cargo test`
- **Non-impacts**: No MCP protocol changes; no database schema migration; no config file format change; no new daemon process.
