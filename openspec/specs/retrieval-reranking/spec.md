# retrieval-reranking Specification

## Purpose
Optional post-retrieval reordering of candidate chunks using a lightweight ONNX cross-encoder (e.g., `bge-reranker`). Opt-in via config to honor the <100 MB idle memory budget; guarded by a 150 MB model-size cap with graceful degradation if the model is missing or too large.
