...
func setupMediaIntegrationTestDB(t *testing.T) (*pgxpool.Pool, *redis.Client) {
...
func TestMediaUploadListDownloadFlow(t *testing.T) {
	db, rdb := setupMediaIntegrationTestDB(t)
	defer db.Close()
	if rdb != nil {
		defer rdb.Close()
	}
	...
	mediaHandler := NewMediaHandler(db, rdb)
	...
	// Device 1 uploads media via mediaHandler.Upload
	mediaPayload := []byte("fake image payload bytes")
	...
	ctxUpload.Request.SetBody(body.Bytes())
	ctxUpload.Request.Header.Set("Authorization", "Bearer "+initToken)
	ctxUpload.Request.Header.Set("Content-Type", "multipart/form-data; boundary="+writer.Boundary())

	uploadCalled := false
	uploadHandler := func(c context.Context, h *app.RequestContext) {
		uploadCalled = true
		mediaHandler.Upload(c, h)
	}
	...
	if ctxUpload.Response.StatusCode() != http.StatusCreated {
		t.Fatalf("expected 201 for media upload, got %d: %s", ctxUpload.Response.StatusCode(), ctxUpload.Response.Body())
	}
	var uploadResp map[string]string
	if err := json.Unmarshal(ctxUpload.Response.Body(), &uploadResp); err != nil {
		t.Fatalf("failed to parse upload response: %v", err)
	}
	mediaID := uploadResp["media_id"]
	if mediaID == "" {
		t.Fatal("expected media_id in upload response")
	}

	// Verify media bundle is stored in DB with correct content type
	bundleStore := model.NewBundleStore(db)
	bundle, err := bundleStore.GetByID(context.Background(), uuid.MustParse(mediaID))
	if err != nil {
		t.Fatalf("failed to get media bundle from DB: %v", err)
	}
	if bundle.ContentType != contentType {
		t.Fatalf("expected content_type %s, got %s", contentType, bundle.ContentType)
	}
	if !bytes.Equal(bundle.EncryptedPayload, mediaPayload) {
		t.Fatal("media bundle payload mismatch")
	}

	// Device 2 lists media bundles via syncHandler.List (media is stored as bundles)
	...
	if listResp[0]["bundle_id"] != mediaID {
		t.Fatalf("expected bundle_id %s, got %v", mediaID, listResp[0]["bundle_id"])
	}
	if listResp[0]["content_type"] != contentType {
		t.Fatalf("expected content_type %s in list, got %v", contentType, listResp[0]["content_type"])
	}

	// Device 2 downloads the media bundle via syncHandler.Download
	ctxDownload := app.NewContext(0)
	ctxDownload.Request.Header.Set("Authorization", "Bearer "+respToken)
	ctxDownload.Params = param.Params{{Key: "id", Value: mediaID}}
	...
	// Verify content type and payload hash match
	downloadedContentType := string(ctxDownload.Response.Header.Peek("X-Syncmind-Content-Type"))
	if downloadedContentType != contentType {
		t.Fatalf("expected downloaded content type %s, got %s", contentType, downloadedContentType)
	}
	downloadedHash := string(ctxDownload.Response.Header.Peek("X-Syncmind-Payload-Hash"))
	expectedHash := sha256.Sum256(mediaPayload)
	expectedHashStr := hex.EncodeToString(expectedHash[:])
	if downloadedHash != expectedHashStr {
		t.Fatalf("expected payload hash %s, got %s", expectedHashStr, downloadedHash)
	}
	if !bytes.Equal(ctxDownload.Response.Body(), mediaPayload) {
		t.Fatal("downloaded media payload mismatch")
	}

	fmt.Println("media flow test passed")
}
